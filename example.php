...
function PrintSemester($cyc, $sem)
{
	...
	foreach ($cyc as $cycle){
		foreach ($cycle as $subject_group) {
			...
			if ((!in_array($subject->get_name(), $PrintedSubject)) && ($subject->work_count_in_semester($sem) > 0))
			{
				...
				if ($JoinRow)
				{
					echo "<tr><td rowspan=".$coll_count.">".$sub_code."</td><td rowspan=".$coll_count.">".$subject->get_name()."</td><td  rowspan=".$coll_count.">".$subject->get_subject_hours()."</td>";//<td rowspan=".$coll_count.">".$sem."</td>";
				}
				$col_count_cur = 0;
				foreach($subject as $sw)
				{
					//if ($sw->get_number()==$sem)
					{
						...
						if ($JoinRow)
						{
							...
							echo "<td>".$sw->get_number()."</td><td>".$sw->get_type_testing()."</td>";
						}
						else
						{
						echo "<tr>";
						if ($tmp_gr == null)
						{
							echo "<td>".$sub_code."</td><td>".$subject->get_name()."</td><td>".$sw->get_hours()."</td><td>".$sw->get_number()."</td><td>".$sw->get_type_testing()."</td>";
						}
						else
						{
							echo "<td><a href='/extranet/workgroups/group/".$tmp_gr["ID"]."/'>".$subject->get_name()."</a></td><td>".$sw->get_number()."</td><td>".$sw->get_hours()."</td><td>".$sw->get_type_testing()."</td>";
						}
						}
						echo "</tr>";
					}
...
<table cellpadding="0" cellspacing="0" class="packages">
	<tr><th colspan="2">Дисциплина (практика)</th><th rowspan="2">Объем (в АЧ)</th><th rowspan="2">Семестр</th><th rowspan="2">Форма промежуточной аттестации</th></tr>
